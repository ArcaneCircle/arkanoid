...
let arkanoidGame, imgBall, imgPaddle, imgBricks, sfxBounce, sfxHit, sfxWin;
let BallDirs = {
  NONE: 0,
  LEFT: 1,
  RIGHT: 2,
...
function ArkanoidGame(canvas, context) {
  ...
  this.init = () => {
    ...
    this.lifesContainer.innerHTML = this.lifes = localStorage.lifes
      ? parseInt(localStorage.lifes)
      : INITIAL_LIFES;
    this.scoreContainer.innerHTML = this.score =
      parseInt(localStorage.score) || 0;
  ...
  this.update = () => {
    if (this.gamePaused || !this.lifes || this.ball.dir === BallDirs.NONE)
      return;
    ...
    if (
      ...
      let maxSpeed =
        BALL_MAX_SPEED - (this.level < 5 ? 2 : this.level < 10 ? 1 : 0);
      if (this.ball.speed < maxSpeed) {
        this.ball.speed +=
          getRandomInt(2, 3) * 0.1 + (this.level < 10 ? 0.1 : 0.2);
      ...
      if (this.ball.dir & BallDirs.DOWN) {
        ...
        this.ball.x +=
          (this.ball.x > this.paddle.x + this.paddle.width / 2 ? -1 : 1) *
          this.ball.radius *
          2;
      }
    }
...
function loadAssets() {
  imgBricks = new Image();
  imgBricks.src = bricksURL;
  imgPaddle = new Image();
  imgPaddle.src = paddleURL;
  imgBall = new Image();
  imgBall.src = ballURL;
